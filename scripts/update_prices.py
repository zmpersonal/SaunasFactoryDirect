...
def page_html(p):
    ...
    offer_html=''.join(
        ...
        f'<div><strong>${fnum(o.get("price")):,.0f}</strong> · <a href="{e(str(o.get("url","")),quote=True)}" target="_blank" rel="noopener">{e(sku)}</a></div></div>'
        for o in offers
    )
